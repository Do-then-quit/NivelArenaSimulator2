import { describe, expect, it } from 'vitest';
import { DUMMY_CARDS } from '../../../src/logic/CardDatabase';
import { GameEngine } from '../../../src/logic/GameEngine';
import { StrongBotV2 } from '../../../src/logic/ai/StrongBotV2';
import { Card, Phase } from '../../../src/logic/types';

function getCard(id: string): Card {
    const card = DUMMY_CARDS.find(c => c.id === id);
    if (!card) throw new Error(`Card ${id} not found`);
    return JSON.parse(JSON.stringify(card));
}

function createEngine(seed: number): GameEngine {
    const deck1 = Array.from({ length: 30 }, (_v, i) => getCard(i % 2 === 0 ? 'ST01-002' : 'BT01-057'));
    const deck2 = Array.from({ length: 30 }, (_v, i) => getCard(i % 2 === 0 ? 'ST01-002' : 'BT01-057'));
    const engine = new GameEngine('P1', 'P2', deck1, deck2, getCard('ST03-001'), getCard('ST03-001'), { seed });
    engine.state.winner = null;
    engine.state.turnPlayerIndex = 0;
    engine.state.phase = Phase.MAIN;
    return engine;
}

function createSearchOnlyBot(name: string): StrongBotV2 {
    const bot = new StrongBotV2(name, {
        beamWidth: 4,
        maxDepth: 3,
        expansionBudget: 100,
        interactionDepth: 3,
        interactionExpansionBudget: 100,
        rolloutVariants: 1,
    });
    (bot as any).fallback = { chooseAction: () => null };
    return bot;
}

describe('ST03 High Value Targeting Regression', () => {
    it('ST03-003 discard interaction chooses low-value own hand card', () => {
        const engine = createEngine(9601);
        const bot = createSearchOnlyBot('Strong-v2-ST03-003');
        const p1 = engine.state.players[0];
        const p2 = engine.state.players[1];

        const lowValue = getCard('ST01-002');
        lowValue.cost = 2;
        lowValue.power = 500;
        lowValue.hit = 1;

        const highValue = getCard('ST01-009');
        highValue.cost = 6;
        highValue.power = 9000;
        highValue.hit = 2;
        p2.hand = [lowValue, highValue];

        const sourceCard = getCard('ST03-003');
        const effect = sourceCard.effects?.[0];
        expect(effect).toBeDefined();
        engine.initiateTargetSelection(effect!, {
            sourceCard,
            player: p1,
            opponent: p2,
            machine: engine,
        } as any);

        expect(engine.state.interactionMode).toBe('SELECT_TARGET');
        expect(engine.state.interactionOwnerPlayerId).toBe(p2.id);

        const action = bot.chooseAction(engine, p2.id);
        expect(action?.type).toBe('SELECT_HAND_TARGET');
        expect(action?.type === 'SELECT_HAND_TARGET' ? action.targetPlayerId : '').toBe(p2.id);
        expect(action?.type === 'SELECT_HAND_TARGET' ? action.handIndex : -1).toBe(0);
    });

    it('ST03-012 first discard step chooses low-value own hand card', () => {
        const engine = createEngine(9602);
        const bot = createSearchOnlyBot('Strong-v2-ST03-012');
        const p1 = engine.state.players[0];
        const p2 = engine.state.players[1];

        const lowValue = getCard('ST01-002');
        lowValue.cost = 3;
        lowValue.power = 100;
        lowValue.hit = 1;

        const highValue = getCard('ST01-009');
        highValue.cost = 2;
        highValue.power = 9000;
        highValue.hit = 2;
        p1.hand = [lowValue, highValue];

        const sourceCard = getCard('ST03-012');
        const effect = sourceCard.effects?.[0];
        expect(effect).toBeDefined();
        engine.initiateTargetSelection(effect!, {
            sourceCard,
            player: p1,
            opponent: p2,
            machine: engine,
        } as any);

        expect(engine.state.interactionMode).toBe('SELECT_TARGET');
        expect(engine.state.interactionOwnerPlayerId).toBe(p1.id);

        const action = bot.chooseAction(engine, p1.id);
        expect(action?.type).toBe('SELECT_HAND_TARGET');
        expect(action?.type === 'SELECT_HAND_TARGET' ? action.targetPlayerId : '').toBe(p1.id);
        expect(action?.type === 'SELECT_HAND_TARGET' ? action.handIndex : -1).toBe(0);
    });

    it('ST03-014 destroy-own-unit step chooses lowest-value own field target', () => {
        const engine = createEngine(9603);
        const bot = createSearchOnlyBot('Strong-v2-ST03-014');
        const p1 = engine.state.players[0];
        const p2 = engine.state.players[1];

        const lowFodder = getCard('ST01-002');
        lowFodder.cost = 1;
        lowFodder.power = 1000;
        lowFodder.hit = 1;
        p1.unitZones[0].unit = lowFodder;

        const midBody = getCard('ST01-006');
        midBody.cost = 3;
        midBody.power = 5000;
        midBody.hit = 1;
        p1.unitZones[1].unit = midBody;

        const highBody = getCard('ST01-009');
        highBody.cost = 6;
        highBody.power = 9000;
        highBody.hit = 2;
        p1.unitZones[2].unit = highBody;

        const sourceCard = getCard('ST03-014');
        const effect = sourceCard.effects?.[0];
        expect(effect).toBeDefined();
        engine.initiateTargetSelection(effect!, {
            sourceCard,
            player: p1,
            opponent: p2,
            machine: engine,
        } as any);

        expect(engine.state.interactionMode).toBe('SELECT_TARGET');
        expect(engine.state.interactionOwnerPlayerId).toBe(p1.id);

        const action = bot.chooseAction(engine, p1.id);
        expect(action?.type).toBe('SELECT_ZONE_TARGET');
        expect(action?.type === 'SELECT_ZONE_TARGET' ? action.targetPlayerId : '').toBe(p1.id);
        expect(action?.type === 'SELECT_ZONE_TARGET' ? action.zoneIndex : -1).toBe(0);
    });
});
