...
describe('DeckBuilderCardPool', () => {
    it('uses the expected pack allow list', () => {
        expect(DECK_BUILDER_ALLOWED_PACKS).toEqual(['ST01', 'ST02', 'ST03', 'ST04', 'ST05', 'ST06', 'ST07', 'ST08', 'ST09', 'ST10', 'ST11', 'BT01', 'BT02', 'BT03', 'BT04', 'BT05', 'BT06', 'SB01']);
    });
    ...
    it('includes BT04 and BT05 cards after pack implementation is complete', () => {
        const cards = getDeckBuilderCards(DUMMY_CARDS);
        expect(cards.some(card => card.id.startsWith('BT04-'))).toBe(true);
        expect(cards.some(card => card.id.startsWith('BT05-'))).toBe(true);
    });

    it('includes ST05, ST06, ST07, ST08, ST09, ST10, ST11, BT02, BT03, BT04, BT05, BT06, and SB01 cards', () => {
        const cards = getDeckBuilderCards(DUMMY_CARDS);
        expect(cards.some(card => card.id.startsWith('ST05-'))).toBe(true);
        expect(cards.some(card => card.id.startsWith('ST06-'))).toBe(true);
        expect(cards.some(card => card.id.startsWith('ST07-'))).toBe(true);
        expect(cards.some(card => card.id.startsWith('ST08-'))).toBe(true);
        expect(cards.some(card => card.id.startsWith('ST09-'))).toBe(true);
