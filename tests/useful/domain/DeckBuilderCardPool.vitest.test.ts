...
describe('DeckBuilderCardPool', () => {
    it('uses the expected pack allow list', () => {
        expect(DECK_BUILDER_ALLOWED_PACKS).toEqual(['ST01', 'ST02', 'ST03', 'ST04', 'ST05', 'ST06', 'ST10', 'ST11', 'BT01', 'BT02', 'BT03', 'BT06', 'SB01']);
    });
    ...
    it('excludes cards from non-allowed packs', () => {
        const cards = getDeckBuilderCards(DUMMY_CARDS);
        expect(cards.some(card => card.id.startsWith('BT04-'))).toBe(false);
    });

    it('includes ST05, ST06, ST10, ST11, BT02, BT03, BT06, and SB01 cards', () => {
        const cards = getDeckBuilderCards(DUMMY_CARDS);
