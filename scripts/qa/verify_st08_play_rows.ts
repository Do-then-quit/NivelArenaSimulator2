...
const SCREENSHOT_DIR = '/tmp/nivelarena-st08-play-verify';
...
async function main() {
    ...
    const packCheckbox = page.locator('input.pack-filter-checkbox[value="ST08"]');
    ...
    if (rowCount !== scenarios.length) {
        throw new Error(`Expected ${scenarios.length} ST08 rows, found ${rowCount}`);
    }
    ...
    for (let index = Math.max(0, START_INDEX - 1); index < scenarios.length; index += 1) {
        ...
        if (diff) {
            const screenshotPath = `${SCREENSHOT_DIR}/failure-${String(index + 1).padStart(2, '0')}.png`;
    ...
    const listScreenshotPath = `${SCREENSHOT_DIR}/st08-play-verify-list.png`;
    await page.screenshot({ path: listScreenshotPath, fullPage: false }).catch(() => {});
    if (failures.length === 0) {
        console.log(`ST08 Playwright scenario replay verification passed: ${scenarios.length}/${scenarios.length}`);
        console.log(`Screenshot: ${listScreenshotPath}`);
    } else {
        console.error(`ST08 Playwright scenario replay verification failed: ${failures[0].displayName} -> ${failures[0].diff}`);
